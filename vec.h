...
#include <ostream>
#include <stdint.h>
/*Реализация вектора для модели - не лучшая реализация, но была готовая*/
#define INF 1e30;
...
public:
    inline Vec()	{}
    inline Vec(const T &x0)	{	_v[0] = x0;	}
    inline Vec(const T &x0, const T &x1)	{	_v[0] = x0;	_v[1] = x1;	}
    inline Vec(const T &x0, const T &x1, const T &x2)	{	_v[0] = x0;	_v[1] = x1;	_v[2] = x2;	}
    inline Vec(const T &x0, const T &x1, const T &x2, const T &x3)	{	_v[0] = x0;	_v[1] = x1;	_v[2] = x2;	_v[3] = x3;	}
...
#undef IMPLEMENT_OP
    ...
    inline T sq() const	{	return *this | *this;	}
    inline T    length() const	{	return std::sqrt(sq());	}
    ...
    inline const T &operator[](size_t idx) const	{	return _v[idx];	}
    inline T &operator[](size_t idx)	{	return _v[idx];	}
    ...
    /*inline bool isinf(uint32_t num)
    {
        if (num > INF)
            return true;
    }
    //inline bool isnan() { return  }*/
private:
    T _v[N];
};
